...
dat[['pep']][['A549']] <- readtable(c(paste0(path, 'tA549_Enrich/peptides.txt')))
dat[['pep']][['Vero']] <- readtable(c(paste0(path, 'tVero_Enrich/peptides.txt')))

samples <- matrix(c(NA, NA), nrow = 1, ncol = 2)

# Load in the TMTpro design matrices containing the randomised layouts
# A549-ACE2
dat[['tmt']][['A549']] <- readmatrix(c(paste0(path, 'SARS2a549tmtlabelling_20200507.csv')))
# VeroE6
dat[['tmt']][['Vero']] <- readmatrix(c(paste0(path, 'Verotmtlabelling_20200511.csv')))
...
for (ii in 1:numel(samples)){
   ...
   # Median normalise to control for protein loading
    ...
    dat[['h24']].(samples[[ii]]) <- dat.mat.(samples[[ii]])(:,[}-5: }])
...
datP[['prot']][['A549']] <- readtable(c(paste0(path, 'tA549_TotalFrac/proteinGroups.txt')))
datP[['prot']][['Vero']] <- readtable(c(paste0(path, 'tVero_TotalFrac/proteinGroups.txt')))
...
tcTicklabels <- matrix(c(NA, NA, NA, NA, NA, NA, NA, NA, NA), nrow = 1, ncol = 8)

# Timecourse: qRT-PCR Data
tc_r_V <- readmatrix(c(paste0(path, 'BjornData/TC_Vero_SupRNA.csv')))'
tc_r_A <- readmatrix(c(paste0(path, 'BjornData/TC_A549_SupRNA.csv')))'

# Timecourse: Plaque Assay Data
tc_p_V <- readmatrix(c(paste0(path, 'BjornData/TC_Vero_Plaque.csv')))'
tc_p_A <- readmatrix(c(paste0(path, 'BjornData/TC_A549_Plaque.csv')))'
...
line(c(paste0(-fccut, -fccut)),[0 -pmax],'Color','k','LineStyle','--')
line(c(paste0(fccut, fccut)),[0 -pmax],'Color','k','LineStyle','--')
line(c(paste0(-xcut, xcut)),[-log10(qcut) -log10(qcut)],'Color','k','LineStyle','--')
#set(gca,'Fontsize',14)
leg}('N-termini','Cellular','Viral','location','northwest')
leg}('boxoff')
...
leg}(matrix(c(NA, NA), nrow = 1, ncol = 4),'Location','southeast')
leg}('boxoff')
xticks(tcTicks)
...
leg}(matrix(c(NA, NA, NA, NA), nrow = 1, ncol = 9),'Location','southeast')
leg}('boxoff')
xticks(tcTicks)
xticklabels(tcTicklabels)
xlim([-1,25])
...
# Save figure
print(c(paste0(path, '/Figures/Fig_Timecourse.pdf')),'-dpdf')
